...
# introduce two new include nodes, basically we want the src and dst nodes to have to be included in this bfs
def BreadthFirstSearchModified(capacity, neighbors, flows, start, end, include1, include2):
  length = len(capacity)
  parents = [-1 for i in xrange(length)] # parent table
  parents[start] = -2 # make sure source is not rediscovered
  M = [0 for i in xrange(length)] # Capacity of path to vertex i
...
if __name__ == "__main__":
  ...
  for line in capacity:
    print line
  print neighbors
  flow, flows = EdmondsKarp(capacity, neighbors, 0, 1)
